...
1. **AI产品管理与优化**
   - 增加对产品的AI管理功能，适合于B2B，用户可以在Admin中新增，查看和管理已导入的产品。(优先考虑集成能Vercel部署)
   - 产品AI优化，标题，详情，场景图
...
1. **接入微信支付**
   - 开户：在微信支付商户平台注册账号，获取商户ID和API密钥。
   - 配置：在项目中配置微信支付的相关参数，包括商户ID、API密钥、通知地址等。
...
2. **AI功能优化**
   - AI Optimizer，会根据当前语言来做优化，增加内外链建议
   - New Post / AI 生成，一键生成满足 schema 要求的英文文章（正文、摘要、结构化数据、SEO、OG）。同时给出内外链建议，帮助你一开始就把链接策略设计好。
...
1. **BLOG优化**
   - UI样式优化
   - 按语言显示
   - 优化图片为supabase存储，避免直接存储在sanity中

2. **New Post AI生成内容优化**
   - 生成内容同时，满足sanity/schemaTypes要求，做好SEO优化
   - 优化视觉内容：为图片和视频添加描述性的文件名和Alt文本，这不仅是SEO要求，也能帮助AI理解多媒体信息
